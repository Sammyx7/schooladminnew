...
export async function getAssignmentOptions(): Promise<{ classes: string[]; sections: string[]; subjects: string[] }> {
  // Source of truth: Admin Settings
  const { getSchoolSettings, listClasses, listSections, listSubjects } = await import('./settingsService');
  const settings = await getSchoolSettings();
  const classes = listClasses(settings);
  const sections = listSections(settings);
  const subjects = listSubjects(settings);
